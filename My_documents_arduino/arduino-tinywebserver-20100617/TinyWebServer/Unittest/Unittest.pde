...
protected:
  ...
  virtual boolean read_next_char(uint8_t* ch) {
...
void test_get_field() {
  {
    char b[] = "GET / HTTP/1.0";
    expect_str_eq("GET", TinyWebServer::get_field(b, sizeof(b), 0));
    expect_str_eq("/", TinyWebServer::get_field(b, sizeof(b), 1));
    expect_str_eq("HTTP/1.0", TinyWebServer::get_field(b, sizeof(b), 2));
  }
  ...
  {
    char b[] = "GET / HTTP/1.0";
    expect_str_eq(NULL, TinyWebServer::get_field(b, sizeof(b), 3));
  }
}
