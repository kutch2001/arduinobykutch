...
{ 
  ...
  if(USE_THERMISTOR) {
    ...
    pinMode (THERMI_O_PIN,INPUT); 
    pinMode(THERMI_O_PWR,OUTPUT); 
    digitalWrite (THERMI_O_PWR,HIGH);
  }
  if(USE_HBP_THERMISTOR) {
    pinMode(TEMP_B_PIN,INPUT); 
    Serial.println ("temp_b");
  }
  else {
    pinMode (THERMI_B_PIN,INPUT); 
    pinMode(THERMI_B_PWR,OUTPUT); 
    digitalWrite (THERMI_B_PWR,HIGH); 
    Serial.println ("THERMI_b");
  }
}
...
{ 
  ...
  if( Serial.available() > 0 ) {
    ...
    if(serial_char == '\n' || serial_char == '\r' || serial_char == ':' || serial_count >= (MAX_CMD_SIZE - 1) ) 
    {
      ...
      //Move echo into Debugging to hopefully improve performance while printing.
      if(DEBUGGING) {
        Serial.print("Echo:");
...
inline bool code_seen(char code_string[]) { 
  return (strstr(cmdbuffer, code_string) != NULL); 
...
#define E_TIME_FOR_MOVE ((float)e_steps_to_take / (e_steps_per_unit*feedrate/60000000))
      ...
      if(DEBUGGING) {
        ...
        Serial.println(x_steps_to_take); 
        Serial.print("X_TIME_FOR_MVE: "); 
        Serial.println(X_TIME_FOR_MOVE); 
        Serial.print("x_interval: "); 
        ...
        Serial.println(y_steps_to_take); 
        Serial.print("Y_TIME_FOR_MVE: "); 
        Serial.println(Y_TIME_FOR_MOVE); 
        Serial.print("y_interval: "); 
        ...
        Serial.println(z_steps_to_take); 
        Serial.print("Z_TIME_FOR_MVE: "); 
        Serial.println(Z_TIME_FOR_MOVE); 
        Serial.print("z_interval: "); 
        ...
        Serial.println(e_steps_to_take); 
        Serial.print("E_TIME_FOR_MVE: "); 
        Serial.println(E_TIME_FOR_MOVE); 
        Serial.print("e_interval: "); 
  ...
  if(code_seen('M'))
  {
    ...
    case 105: // M105
      ...
      if (USE_THERMISTOR) {
        Serial.print( analog2temp(TEMP_0_PIN) );
      }
      else {
        Serial.print( analog2temp(THERMI_O_PIN) );
      }
      //get platform temp
      Serial.print(" B:");
      if (USE_HBP_THERMISTOR) {
        Serial.println( analog2temp(TEMP_B_PIN) ); 
      }
      else {
        Serial.println( analog2temp(THERMI_B_PIN) ); 
      }
...
void linear_move(unsigned long x_steps_remaining, unsigned long y_steps_remaining, unsigned long z_steps_remaining, unsigned long e_steps_remaining) // make linear move with preset speeds and destinations, see G0 and G1
{
  ...
  if(DEBUGGING) {
    ...
    {
      Serial.print ("E_DIR_PIN"); 
      Serial.print (E_DIR_PIN);
      Serial.print (" !INVERT_E_DIR"); 
      Serial.print (!INVERT_E_DIR);
      Serial.println("");
    }
    else
  ...
  if(DEBUGGING) {
    ...
    Serial.print (" X_MIN_PIN "); 
    Serial.print (X_MIN_PIN); 
    Serial.print (" ENDSTOPS_INVERTING "); 
    Serial.print (ENDSTOPS_INVERTING); 
    Serial.println ("");
    Serial.print ("Y_MIN_PIN "); 
    Serial.print (Y_MIN_PIN); 
    Serial.print (" direction_y "); 
    Serial.print (direction_y); 
    Serial.print (" Y_MIN_PIN "); 
    Serial.print (Y_MIN_PIN); 
    Serial.print (" ENDSTOPS_INVERTING "); 
  ...
  if(DEBUGGING) {
    ...
    Serial.print ("x_steps_remaining "); 
    Serial.print (x_steps_remaining); 
    Serial.println ("");
    Serial.print ("y_steps_remaining "); 
    Serial.print (y_steps_remaining); 
    Serial.println ("");
    Serial.print ("z_steps_remaining "); 
    Serial.print (z_steps_remaining); 
    Serial.println ("");
    Serial.print ("e_steps_remaining "); 
    Serial.print (e_steps_remaining); 
    Serial.println ("");
  };
  ...
  { 
    ...
    if(e_steps_remaining) if ((micros()-previous_micros_e) >= e_interval) { 
      do_e_step(); 
      e_steps_remaining--; 
    }
...
inline void disable_x() { 
  if(X_ENABLE_PIN > -1) digitalWrite(X_ENABLE_PIN,!X_ENABLE_ON); 
}
inline void disable_y() { 
  if(Y_ENABLE_PIN > -1) digitalWrite(Y_ENABLE_PIN,!Y_ENABLE_ON); 
}
inline void disable_z() { 
  if(Z_ENABLE_PIN > -1) digitalWrite(Z_ENABLE_PIN,!Z_ENABLE_ON); 
}
inline void disable_e() { 
  if(E_ENABLE_PIN > -1) { 
    if (USE_MOTOR == false) { 
      digitalWrite(E_ENABLE_PIN,!E_ENABLE_ON); 
    } 
  }; 
}
...
int get_extruder_temperature () {
  ...
  if (USE_THERMISTOR) {
    return analog2temp (TEMP_0_PIN);
  }
  else {
    return analog2temp (THERMI_O_PIN);
  }
}
...
int get_platform_temperature () {
  ...
  if (USE_HBP_THERMISTOR) {
    return analog2temp (TEMP_B_PIN);
  }
  else {
    return analog2temp (THERMI_B_PIN);
  }
}
...
{ 
  ...
  while (oldT < target_var)
  {
    ...
    if (USE_THERMISTOR) {
      Serial.println( analog2temp(TEMP_0_PIN) );
    }
    else {
      Serial.println( analog2temp(THERMI_O_PIN) );
    }
    previous_millis_heater = millis(); 
    delay(1000);
  }
...
inline void kill(byte debug)
{
  ...
  while(1)
  {
    ...
    {
    case 1: 
      Serial.print("Inactivity Shutdown, Last Line: "); 
      break;
    case 2: 
      Serial.print("Linear Move Abort, Last Line: "); 
      break;
    case 3: 
      Serial.print("Homing X Min Stop Fail, Last Line: "); 
      break;
    case 4: 
      Serial.print("Homing Y Min Stop Fail, Last Line: "); 
      break;
    } 
    Serial.println(gcode_LastN);
